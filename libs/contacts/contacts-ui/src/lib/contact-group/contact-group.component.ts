...
export class ContactGroupPageComponent {
  ...
  isNew = computed(() => this.resolvedId()?.isNew ?? false);
  ...
  async onSave(group: ContactGroup): Promise<void> {
    ...
    if (isNew) {
      await this.state.createGroup(
        group.name,
        group.description || '',
        group.memberUrns,
      );
    } else {
      // [Update] Save local metadata + members
  ...
  private getNewGroup(urn: URN): Observable<ContactGroup> {
    this.subGroups.set([]);
    // ✅ FIX: Return V9 compliant object
    return of({
      id: urn,
      // directoryId is undefined for new local groups
