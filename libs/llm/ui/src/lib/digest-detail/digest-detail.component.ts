...
export class LlmDigestDetailComponent {
  ...
  async saveEdit() {
    ...
    try {
      ...
      await this.digestStorage.saveDigest(updated);
      this.source.refresh();
  ...
  async openContextPreview() {
    ...
    try {
      // Fetch the raw history needed to reconstruct the context
      ...
      this.dialog.open(LlmDigestPreviewDialogComponent, {
        ...
        data: {
          ...
          includeRawProposals: false, // Defaulting to false for safety/speed on historical view
          systemPrompt:
