...
import { ConversationIndexRecord } from '../db/records/conversation.record';
...
export class ChatStorageService {
  ...
  private quarantineMapper = inject(QuarantineMapper);
  ...
  async saveQuarantinedMessage(message: TransportMessage): Promise<void> {
    const record = this.quarantineMapper.toRecord(message);
  ...
  async getQuarantinedMessages(senderId: URN): Promise<ChatMessage[]> {
    ...
    return records.map((r) => this.quarantineMapper.toDomain(r));
  }
