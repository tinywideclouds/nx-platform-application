...
1.  **`ChatDataService` (The "Query" Service):** Responsible for _pulling_ data from the message queue and acknowledging receipt.
2.  **`ChatSendService` (The "Command" Service):** Responsible for _pushing_ new messages to the send endpoint.
...
Run `nx test chat-access` to execute the unit tests for this library.
