...
vi.mock('passport-google-oauth20', () => {
    ...
    class FakeStrategy {
        name = 'google';
        _verify: (req: any, accessToken: string, refreshToken: string, params: any, profile: any, done: (error: any, user?: any) => void) => void;
        ...
        authenticate(req: any) {
            ...
            this._verify(req, 'mock-access-token', 'mock-refresh-token', params, userProfile, done);
        }
    }
...
describe('Authentication Flow (Integration)', () => {
    ...
    beforeAll(async () => {
        ...
        console.log('[TEST LOG] Test server and emulator started for Authentication Flow.');
        db = new Firestore({ projectId: 'test-project' });
    },60000);

    afterAll(async () => {
        if (!testServer)
            throw(new Error("test server is undefined"))
    ...
    it('should authenticate an authorized user and issue a JWT', async () => {
        ...
        const loginResponse = await agent.get('/auth/google/callback');
        ...
        expect(statusResponse.body.user.token).toBeDefined();
        expect(statusResponse.body.user.token.length).toBeGreaterThan(20);
    }, 30000);
});
